...
class AesFromScratch:
    ...
    def initTables(self):
        ...
        self.RCON = ["01", "02", "04", "08", "10", "20", "40", "80", "1b", "36","6c","d8","ab","4d","9a","2f","5e","bc"]
    ...
    def keySchedule(self, key, round):
        words = []
        roundKey = [None, None, None, None, None, None, None, None, None, None, None, None]

        while key:
            words.append(key[:8])
            key = key[8:]

        roundKey[0] = self.xor(words[0], self.SboxMatch(words[11], round)).zfill(8)
    ...
    def encrypt16round(self, plaintext, key):
        ...
        print("Initial Key:")
        rklist = textwrap.wrap(key, 2)
        my_matrix = np.array(rklist).reshape((4, 12)).T
        print(my_matrix)

        # Initial Key Addition Layer
        state = self.keyAddition(state, key)
        print(" After first key addition: ")
        my_matrix = np.array(state).reshape((4, 12)).T
        print(my_matrix)
        initialise=1
...
if __name__ == "__main__":
    while True:
        ...
        key = "2B7E151628AED2A6ABF7158809CF4F3C6ABF7158809CF4F36ABF7158809CF4F36ABF7158809CF4F36ABF7158809CF4F3"
