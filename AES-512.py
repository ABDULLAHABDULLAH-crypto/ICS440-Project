...
class AesFromScratch:
    ...
    def initTables(self):
        ...
        self.RCON = ["01", "02", "04", "08", "10", "20", "40", "80", "1b", "36","6c","d8","ab","4d","9a","2f","5e","bc","63","c6","97","35"]
    ...
    def keySchedule(self, key, round):
        words = []
        roundKey = [None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None]

        while key:
            words.append(key[:8])
            key = key[8:]

        roundKey[0] = self.xor(words[0], self.SboxMatch(words[14], round)).zfill(8)
        ...
        roundKey[4] = self.xor(words[4], roundKey[3]).zfill(8)
        roundKey[5] = self.xor(words[5], roundKey[4]).zfill(8)
        roundKey[6] = self.xor(words[6], roundKey[5]).zfill(8)
        ...
        roundKey[10] = self.xor(words[10], roundKey[9]).zfill(8)
        roundKey[11] = self.xor(words[11], roundKey[10]).zfill(8)
        roundKey[12] = self.xor(words[12], roundKey[11]).zfill(8)
        roundKey[13] = self.xor(words[13], roundKey[12]).zfill(8)
        roundKey[14] = self.xor(words[14], roundKey[13]).zfill(8)
        roundKey[15] = self.xor(words[15], roundKey[14]).zfill(8)
    ...
    def keyAddition(self, state, key):
        ...
        if(len(keyString)>=48):
            keyString=self.xor(keyString[:32], keyString[96:]).zfill(len(stateString))
    ...
    def encrypt16round(self, plaintext, key):
        ...
        # print("Initial Key:")
        # rklist = textwrap.wrap(key, 2)
        # my_matrix = np.array(rklist).reshape((4, 16)).T
        # print(my_matrix)

        # # Initial Key Addition Layer
        # state = self.keyAddition(state, key)
        # print(" After first key addition: ")
        # my_matrix = np.array(state).reshape((4, 16)).T
        # print(my_matrix)
        initialise=1
...
if __name__ == "__main__":
    ...
    key = "2B7E151628AED2A6ABF7158809CF4F3C6ABF7158809CF4F36ABF7158809CF4F36ABF7158809CF4F36ABF7158809CF4F36ABF7158809CF4F36ABF7158809CF4F3"
    ...
    print("\n\nTime for AES-512:\t",(end_time-start_time))
